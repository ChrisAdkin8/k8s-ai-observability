...
- CI must be green, and `fast` gates the five expensive jobs, so a broken rule test costs
  seconds rather than the ~16 runner-minutes they would otherwise spend confirming it.
  Per-job timings are in [docs/ci.md](docs/ci.md), which owns them; quote the run id with
  any figure you take from there.
  ...
  **DONE — every one of those was wrong. Re-derived 2026-08-04 from run 30870290833.
  ...
  ⚠️ ~~This bullet kept its own copy of the figures, and quoted run 30870290833's total as
  6m50s.~~ **DONE — corrected and de-duplicated 2026-08-05. That run's total was 6m27s
  under every way of measuring it (job span, created-to-last-job, and the API's wall clock
  agree within 2s and reproduce the other run's 5m27s exactly). The per-job numbers here
  were right; only the total was invented, which is what a second copy buys you.**
